...
class RegPageData:
    """Данные для тестирования страницы регистрации/логина."""

    VALID_LIST = reader.get_reg_page_valid()
    INVALID_LIST = reader.get_reg_page_invalid()

    VALID = (
        VALID_LIST[0] if VALID_LIST else {"username": "angular", "password": "password"}
    )
    INVALID = (
        INVALID_LIST[0]
        if INVALID_LIST
        else {"username": "wrong_user", "password": "wrong_password"}
    )


class BankLoginData:
    """Данные для тестирования логина банковского приложения."""
...
class ManagerPageData:
    """Данные для тестирования страницы менеджера банка."""
